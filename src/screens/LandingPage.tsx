...
const features = [
  ...
  },
  {
    ...
    desc: "OTP login — no passwords ever. Phone numbers stay hidden from other users.",
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      >
        ...
        {/* Headline */}
        <h1 className="animate-fade-in-up-1 text-4xl font-black text-white leading-tight tracking-tight mb-4">
          Your daily commute,{" "}
        ...
        <p className="animate-fade-in-up-2 text-base text-blue-200 leading-relaxed mb-8 max-w-xs">
        ...
        {/* CTAs */}
        <div className="animate-fade-in-up-3 flex flex-col gap-3 mb-10">
          <button
            onClick={() => navigate("/login")}
            className="w-full py-4 rounded-2xl font-bold text-base text-brand-900 transition-all active:scale-95"
          ...
          <button
            ...
            className="w-full py-4 rounded-2xl font-semibold text-sm text-white border border-white/20 bg-white/5 active:bg-white/10"
          >
            See How It Works
          </button>
        </div>
        ...
        <div className="animate-fade-in-up-4 bg-white/10 border border-white/15 rounded-2xl px-4 pt-4 pb-2 backdrop-blur-sm">
        ...
        {/* Stats row */}
        <div className="animate-fade-in-up-5 grid grid-cols-3 gap-3 mt-5">
          {[
        ...
        <div className="space-y-6">
          {steps.map((step, i) => (
            <div key={i} className="flex gap-4 items-start">
              <div className="shrink-0 w-12 h-12 rounded-2xl bg-brand-700 flex items-center justify-center text-white shadow-lg shadow-brand-700/30">
                {step.icon}
              </div>
              <div className="pt-1">
      ...
      <section id="features" className="bg-white px-6 py-14">
        ...
        <h2 className="text-2xl font-black text-gray-900 mb-8">Built for your commute</h2>

        <div className="grid grid-cols-2 gap-4">
          {features.map((f, i) => (
            <div
              key={i}
              className="rounded-2xl border border-gray-100 p-4 flex flex-col gap-3 shadow-sm hover:shadow-md transition-shadow"
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${f.color}`}>
                {f.icon}
              </div>
              <div>
                <h3 className="text-sm font-bold text-gray-900 leading-tight mb-1">{f.title}</h3>
      ...
      {/* ── Fare Highlight ── */}
      <section className="mx-6 mb-8 rounded-3xl px-6 py-8 text-white overflow-hidden relative"
        ...
        <div className="flex flex-col gap-2">
          {["No hidden charges", "Pay cash to driver", "No payment gateway needed"].map((item) => (
            <div key={item} className="flex items-center gap-2">
              <div className="w-4 h-4 rounded-full bg-green-400 flex items-center justify-center shrink-0">
                <svg viewBox="0 0 24 24" className="w-2.5 h-2.5 text-green-900" fill="none" stroke="currentColor" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round">
      ...
      >
        ...
        <p className="text-sm text-blue-300 mb-8 leading-relaxed">
          Join your neighbours already sharing the GC ↔ HCL commute.
          Sign up in under a minute — just your phone number.
        </p>
        <button
          onClick={() => navigate("/login")}
          className="w-full py-4 rounded-2xl font-bold text-base text-brand-900 transition-all active:scale-95 mb-3"
          style={{ background: "linear-gradient(90deg, #fbbf24, #f59e0b)" }}
        >
          Start Carpooling Now
        </button>
        <p className="text-xs text-blue-400">Free to use · No credit card · No passwords</p>
      </section>
